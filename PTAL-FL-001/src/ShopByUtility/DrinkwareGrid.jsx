...
//       (item) => item.class.toLowerCase() === "drinkware"
//     );
    ...
    <div className="bg-[#f9f4ec] text-[#1e1e1e] rounded-md overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-300 relative">
      ...
      <div className="p-4 flex flex-col gap-2 pb-12">
        ...
        <div className="text-[#8b0000] text-sm flex items-center gap-2">
          ...
          <span>{product.reviews} {t('drinkware_grid.reviews')}</span>
        </div>
      ...
      >
        {isAdded ? t('drinkware_grid.added_to_cart') : t('drinkware_grid.add_to_cart')}
      </button>
    </div>
  );
};
...
export default function DrinkwareGrid() {
  ...
  const sortOptions = [
    { key: "Featured", label: t('drinkware_grid.sort_options.featured') },
    { key: "Alphabetically, A-Z", label: t('drinkware_grid.sort_options.alphabetically_a_z') },
    { key: "Alphabetically, Z-A", label: t('drinkware_grid.sort_options.alphabetically_z_a') },
    { key: "Price, low to high", label: t('drinkware_grid.sort_options.price_low_to_high') },
    { key: "Price, high to low", label: t('drinkware_grid.sort_options.price_high_to_low') },
  ];

  useEffect(() => {
    const drinkwareProducts = productData.filter(
      (item) => item.class.toLowerCase() === "drinkware"
    );
    setProducts(drinkwareProducts);
  }, []);
    ...
    <section className={`bg-[#f9f4ec] py-12 px-6 md:px-12 transition-all duration-300 ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <h2 className="text-lg font-semibold">
          {products.length} {t('drinkware_grid.products')}
        </h2>
        <select
          value={sortType}
          onChange={(e) => {
          ...
          className="border border-gray-300 rounded px-4 py-2 bg-white text-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-[#8b0000] transition-all"
        >
          <option disabled>{t('drinkware_grid.sort')}</option>
          {sortOptions.map((option) => (
