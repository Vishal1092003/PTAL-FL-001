  ...
  const LoginHandler = async () => {
    ...
    try {
      ...
      if (!res.ok || (data && data.success === false)) {
        // If your API returns { success: false, message: "wrong username" }
        const msg = (data && data.message) || "Login failed";
        throw new Error(msg);
      }

      // At this point we assume data.success === true and data.token exists
      const { token } = data;
      localStorage.setItem("authToken", token);

      toast.success(data.message || "Logged in successfully!");

      // Give the user a moment to read the toast
      setTimeout(() => navigate("/"), 1500);

    } catch (err) {
      // This will show your API error message, e.g. "wrong username"
